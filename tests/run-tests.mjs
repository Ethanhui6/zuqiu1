...
const migrated=migrateLegacy({name:'旧球员',age:25,pos:'CAM',ovr:78,potential:86,attrs:{pac:75,sho:72,pas:84,dri:82,def:45,phy:64},clubId:'CHN1-SHA',fans:10000,stats:{apps:120,goals:32,assists:44}});assert.equal(migrated.schemaVersion,21);assert.equal(migrated.player.name,'旧球员');assert.equal(migrated.career.careerStats.apps,120);
