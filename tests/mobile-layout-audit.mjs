...
try{
  ...
  await page.locator('input[name="name"]').fill('验收球员');await page.locator('[data-next]').click();
  ...
  for(const route of ['career','training','transfer','world','more']){await page.locator(`[data-route="${route}"]`).click();assert.ok(await page.locator(`[data-route="${route}"].active`).isVisible())}
  await page.locator('[data-route="transfer"]').click();assert.equal(await page.locator('.world-map-shell').count(),0);await page.locator('[data-open-world]').click();assert.ok(await page.locator('.world-page').isVisible());assert.match(await page.locator('.page-title').textContent(),/足球世界/);
  await page.locator('[data-route="training"]').click();await page.locator('[data-plan]').nth(1).click();
  ...
  await page.locator('[data-top-speed]').click();await page.locator('[data-sim="nextMatch"]').click();await page.waitForTimeout(250);await page.locator('[data-route="match"]').click();assert.equal(await page.locator('[data-match-interaction]').count(),8);await page.locator('[data-match-interaction="penalty"]').click();await page.locator('[data-play]').click();await page.locator('.sheet [data-home]').waitFor();
...
console.log(JSON.stringify({status:'PASS',engine:'current index.html via system Chromium',viewports:results,flow:['six-step player creation without manual age or birthday','bottom navigation','world and transfer separation','direct training settlement','eight match interactions','sheet cleanup','save reload'],screenshots:path.relative(process.cwd(),shots),physicalSafari:false},null,2));
