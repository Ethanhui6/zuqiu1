...
try{
  ...
  await page.locator('[data-route="match"]').click();assert.equal(await page.locator('[data-match-interaction]').count(),0);const playGeometry=await page.locator('[data-play]').evaluate(button=>{const box=button.getBoundingClientRect(),bar=button.closest('.match-fixed-action')?.getBoundingClientRect(),nav=document.querySelector('.glass-tabbar')?.getBoundingClientRect();return {visible:box.top>=0&&box.bottom<=innerHeight,barVisible:bar?.top>=0&&bar?.bottom<=innerHeight,clearOfNav:!bar||!nav||bar.bottom<=nav.top};});assert.equal(playGeometry.visible,true);assert.equal(playGeometry.barVisible,true);assert.equal(playGeometry.clearOfNav,true);await page.locator('[data-play]').evaluate(button=>button.click());await page.locator('.sheet [data-match-strategy]').first().waitFor();await page.locator('.sheet [data-match-strategy]').first().evaluate(button=>button.click());await page.locator('.sheet [data-mini-pitch-host]').waitFor();await page.locator('.sheet [data-home]').waitFor();
