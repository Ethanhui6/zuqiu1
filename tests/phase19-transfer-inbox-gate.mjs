...
try {
  ...
  for (const viewport of [{ width: 390, height: 844 }, { width: 1440, height: 900 }]) {
    ...
    const generated = await page.evaluate(async () => {
      ...
      state.random.seed = 'phase-6-transfer-world';
    ...
    const overview = page.locator('[data-market-heat]');
    assert.equal(await overview.count(), 1);
    assert.match(await overview.innerText(), /Market Heat\s+(热门|抢手).*球队兴趣.*合同状态.*经纪人建议.*近期传闻/s);
    assert.equal(await overview.evaluate(element => element.getBoundingClientRect().bottom <= window.innerHeight), true);
    const receivedText = await page.locator('.transfer-inbox-panel').innerText();
    for (const stage of ['球探关注', '转会传闻', '经纪人联系', '俱乐部兴趣', '正式报价']) assert.match(receivedText, new RegExp(stage));
    ...
    await page.locator('.app-main').evaluate(element => element.scrollTo(0, 0));
    await page.waitForTimeout(350);
    fs.mkdirSync(path.resolve('test-results'), { recursive: true });
    await page.screenshot({ path: path.resolve(`test-results/phase6-transfer-world-${viewport.width}.png`) });
    await page.locator('[data-transfer-tab="received"]').click();
    await page.locator('[data-inbox-offer]').first().click();
    const negotiationChoices = page.locator('[data-negotiation-choice]');
    assert.equal(await negotiationChoices.count(), 3);
    const probabilities = await negotiationChoices.evaluateAll(elements => elements.map(element => Number(element.dataset.probability)));
    assert.ok(probabilities[0] > probabilities[1] && probabilities[1] > probabilities[2]);
    assert.match(await page.locator('[data-transfer-negotiation]').innerText(), /成功率.*风险.*潜在收益.*失败风险/s);
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > document.documentElement.clientWidth), false);
    await page.waitForTimeout(350);
    await page.screenshot({ path: path.resolve(`test-results/phase7-transfer-negotiation-${viewport.width}.png`) });
    await negotiationChoices.nth(1).click();
    await page.locator('[data-result-ack]').click();
    await page.locator('[data-transfer-tab="history"]').click();
    assert.match(await page.locator('.transfer-inbox-panel').innerText(), /要求(轮换|主力|核心|绝对核心|建队核心).*?(已达成|未达成)/s);
    assert.deepEqual(errors, []);
    results.push({ viewport: `${viewport.width}x${viewport.height}`, stages: generated.stages, offers: generated.offers, heat: await overview.getAttribute('data-market-heat'), probabilities });
    await page.close();
  }
