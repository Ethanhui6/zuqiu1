...
test('match interactions expose sixteen deterministic position-aware checks', () => {
  assert.equal(MATCH_INTERACTIONS.length, 16);
  const player = { ovr: 60, stats: { shooting: 80, passing: 60, dribbling: 60, defending: 60, physical: 60 } };
  const result = resolveMatchInteraction({ id: 'penalty', player, seed: 0, input: { score: 90 } });
  assert.equal(result.option.name, '点球瞄准');
