...
const active=[
  ...
  '../src/components/clubDetailSheet.js',
...
test('active UI emits only the V20 architecture class family',()=>{
  ...
  for(const legacy of ['AppHeader','MainViewport','BottomNavigation','glass-card','career-overview','v19-',"className:'tag'","className:'tag ","className:'tag-row'","className:'scroll-hint ","className:'scroll-hint__chevron'"])assert.doesNotMatch(source,new RegExp(legacy),legacy);
...
test('career home exposes identity console growth and action sections',()=>{
  ...
  for(const section of ['identity','growth','actions'])assert.match(career,new RegExp(`data-section':'${section}`),section);
  assert.match(career,/dataset:\{section:'console'\}/);
  for(const extra of ['v20-home-twin','v20-training-reminder','v20-pace-card','v20-recent-summary'])assert.doesNotMatch(career,new RegExp(extra),extra);
...
test('V20 CSS keeps cards compact and avoids override patches',()=>{
  ...
  for(const dead of ['v20-player-snapshot','v20-career-pair','v20-player-avatar','v20-season-ring'])assert.doesNotMatch(css,new RegExp(dead),dead);
  assert.doesNotMatch(read('../src/styles/v20-product.css'),/\.v20-(?:app-shell|header-avatar|preview-date|club-hero|review-quote)[^{]*\{[^}]*gradient/i);
