...
try{for(const width of [320,390,430]){const page=await browser.newPage({viewport:{width,height:800},hasTouch:true});const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto(`http://127.0.0.1:${server.address().port}/?no-sw=1`,{waitUntil:'networkidle'});const result=await page.evaluate(()=>{const root=document.querySelector('#overlay-root');return{root:Boolean(root),blockers:[...root.children].filter(x=>getComputedStyle(x).pointerEvents!=='none').length}});assert.equal(result.root,true);assert.equal(result.blockers,0);assert.deepEqual(errors,[]);await page.close()}}finally{await browser.close();await new Promise(resolve=>server.close(resolve))}
console.log(JSON.stringify({status:'PASS',productionOverlayRoot:true,interactiveBlockers:0},null,2));
