...
function eventEligible(event,save){
  ...
  if(c.minPotential!==undefined&&save.player.potential<Number(c.minPotential))return false;if(c.maxPotential!==undefined&&save.player.potential>Number(c.maxPotential))return false;
  if(c.minCoachTrust!==undefined&&save.status.coachTrust<Number(c.minCoachTrust))return false;
  if(c.minFitness!==undefined&&save.status.fitness<Number(c.minFitness))return false;if(c.maxFatigue!==undefined&&save.status.fatigue>Number(c.maxFatigue))return false;
  if(c.minMorale!==undefined&&save.status.morale<Number(c.minMorale))return false;if(c.minSeason!==undefined&&save.career.season<Number(c.minSeason))return false;if(c.maxSeason!==undefined&&save.career.season>Number(c.maxSeason))return false;
  if(Array.isArray(c.months)&&c.months.length&&!c.months.includes(save.career.month))return false;
  if(c.minRating!==undefined&&Number(save.career.seasonStats.rating||0)<Number(c.minRating))return false;
  if(c.maxContractYears!==undefined&&Number(save.career.contract.years||0)>Number(c.maxContractYears))return false;
  if(c.careerStrategy&&save.career.strategies?.career!==c.careerStrategy)return false;
...
function eventFingerprint(event,person=''){return [event.category,person||PERSON_BY_CATEGORY[event.category]||'职业环境',choiceSignature(event.choices||[]),event.pressure||'中压',(event.tags||[]).slice(0,2).sort().join('+'),titleKey(event.templateTitle||event.title)].join('|')}
function ensureMemory(save){
  save.career.eventMemory??={};const mem=save.career.eventMemory;
  mem.triggered??=[];mem.recentTags??=[];mem.recentCategories??=[];mem.recentTitles??=[];mem.recentTemplateTitles??=[];mem.recentChoiceSignatures??=[];mem.recentFingerprints??=[];mem.choices??=[];mem.chainsOpen??=[];mem.chainsStarted??=[];mem.chainsClosed??=[];mem.cooldowns??={};mem.recentEventIds??=[];mem.recentPersons??=[];mem.recentOpponents??=[];mem.typeCounts??={};mem.generatedCount??=0;mem.duplicateCount??=0;mem.lastAvailableCount??=0;mem.lastFilteredCount??=0;
  return mem;
}
...
function categoryWeights(save,allowed,mem){
  ...
  return allowed.map(category=>{
    ...
    weight*=Number(save.career.aiDirector?.eventBias?.[category]||1);
...
function bridgeEvent(save,category,rng){
  ...
  return normalizeEvent({
    ...
    choices:[
      ...
      {id:`bridge-${category}-review`,text:'与团队重新评估',style:'longterm',focus:'dri',base:.7,effects:{xp:22,trust:1,morale:4,fitness:2,fans:60},delayedEffects:[{type:'coachTrust',amount:2}]}
    ]
  });
}
...
export async function generateEvent(save,repo,{category:forcedCategory=null}={}){
  ...
  const mem=ensureMemory(save),stageCategories=CATEGORY_BY_STAGE[stage(save)]||CATEGORY_BY_STAGE.成长期,allowed=forcedCategory?[forcedCategory]:stageCategories,recentCategories=mem.recentCategories.slice(-5),recentTitles=new Set(mem.recentTitles.slice(-20)),recentTemplateTitles=new Set(mem.recentTemplateTitles.slice(-20)),recentEventIds=new Set(mem.recentEventIds.slice(-20)),triggeredIds=new Set(mem.triggered);
  const lastCategoryPair=mem.recentCategories.slice(-2),blockedCategory=!forcedCategory&&lastCategoryPair.length===2&&lastCategoryPair[0]===lastCategoryPair[1]?lastCategoryPair[0]:null;
  const activeStories=forcedCategory?[]:(repo.storyChains||[]).map(normalizeEvent).filter(event=>eventEligible(event,save)&&event.prerequisite?.some(token=>mem.chainsOpen.includes(token))&&event.category!==blockedCategory);
  ...
  let categoryCandidates=categoryWeights(save,allowed,mem);if(blockedCategory&&categoryCandidates.length>1)categoryCandidates=categoryCandidates.filter(item=>item.category!==blockedCategory);let category=rng.weighted(categoryCandidates,x=>x.weight)?.category||allowed[0];
  ...
  if(fresh.length)pool=fresh;
    ...
    pool=[];
    const alternatives=rng.shuffle(allowed.filter(x=>x!==category&&x!==blockedCategory));
  ...
  if(!pool.length&&!forcedCategory){
    const fallbacks=[...new Set([...allowed,'training','academy','match','life'])].filter(item=>item!==blockedCategory);
...
function addDelayed(save,choice,event,outcome,rng){
  ...
  const hidden=choice.hiddenEffects&&Object.keys(choice.hiddenEffects).length?choice.hiddenEffects:((['gamble','aggressive','negotiate','longterm'].includes(choice.style)&&rng.bool(.2))?{[outcome.label==='出现代价'?'morale':'coachTrust']:outcome.label==='出现代价'?-3:2}:null);
  if(hidden){
    save.career.hiddenConsequences??=[];
    for(const [type,amount] of Object.entries(hidden)){
      const item={id:`HC-${event.id}-${choice.id}-${type}-${save.career.season}`,dueSeason:save.career.season+rng.int(1,2),type,amount:Number(amount)||0,source:event.title,hidden:true};
      pending.push(item);save.career.hiddenConsequences.push({...item,revealed:false});
    }
    save.career.hiddenConsequences=save.career.hiddenConsequences.slice(-120);
  }
}
...
export function resolveEventChoice(save,choiceId){
  ...
  const mem=ensureMemory(save);if(!mem.triggered.includes(event.id))mem.triggered.push(event.id);mem.recentTags.push(event.category,...event.tags.slice(0,2));mem.recentTags=mem.recentTags.slice(-20);mem.recentCategories.push(event.category);mem.recentCategories=mem.recentCategories.slice(-20);mem.recentTitles.push(titleKey(event.title));mem.recentTitles=mem.recentTitles.slice(-20);mem.recentTemplateTitles.push(titleKey(event.templateTitle||event.title));mem.recentTemplateTitles=mem.recentTemplateTitles.slice(-20);mem.recentEventIds.push(event.id);mem.recentEventIds=mem.recentEventIds.slice(-20);mem.recentPersons.push(event.person||PERSON_BY_CATEGORY[event.category]||'职业环境');mem.recentPersons=mem.recentPersons.slice(-20);const signature=choiceSignature(event.choices);mem.recentChoiceSignatures.push(signature);mem.recentChoiceSignatures=mem.recentChoiceSignatures.slice(-30);mem.recentFingerprints.push(event.fingerprint||eventFingerprint(event,event.person));mem.recentFingerprints=mem.recentFingerprints.slice(-30);mem.choices.push({eventId:event.id,title:event.title,category:event.category,choiceId,label:choice.text,outcome:outcome.label,season:save.career.season,month:save.career.month,week:save.career.calendar?.week||1});mem.choices=mem.choices.slice(-800);mem.cooldowns[event.id]=save.career.season+event.cooldown;mem.lastChoiceSignature=signature;
  if(event.chainId&&event.prerequisite?.length)mem.chainsOpen=mem.chainsOpen.filter(token=>!event.prerequisite.includes(token));
  if(event.chainId&&!mem.chainsStarted.includes(event.chainId))mem.chainsStarted.push(event.chainId);if(!event.chainId&&choice.unlockChain&&!mem.chainsStarted.includes(choice.unlockChain))mem.chainsStarted.push(choice.unlockChain);if(event.next&&!mem.chainsOpen.includes(event.next))mem.chainsOpen.push(event.next);if(choice.unlockChain&&!mem.chainsOpen.includes(choice.unlockChain))mem.chainsOpen.push(choice.unlockChain);if(choice.closeChain){mem.chainsOpen=mem.chainsOpen.filter(x=>x!==choice.closeChain);if(!mem.chainsClosed.includes(choice.closeChain))mem.chainsClosed.push(choice.closeChain)}
  ...
  addDelayed(save,choice,event,outcome,rng);event.resolved=true;event.selectedChoice=choice;event.outcome=outcome;event.growth=growth;save.rng=rng.snapshot();return{event,choice,outcome,growth};
}
...
export function applyDelayedEffects(save){
  ...
  for(const e of due){if(e.type==='reputation')save.fans.mediaHeat=clamp(save.fans.mediaHeat+Number(e.amount||0),0,100);if(e.type==='fans')applyFanChange(save,{social:Number(e.amount||0),global:Math.round(Number(e.amount||0)*.18),reason:e.source||'延迟影响'});if(e.type==='morale')save.status.morale=clamp(save.status.morale+Number(e.amount||0),0,100);if(e.type==='coachTrust'||e.type==='trust')save.status.coachTrust=clamp(save.status.coachTrust+Number(e.amount||0),0,100);if(e.type==='fitness')save.status.fitness=clamp(save.status.fitness+Number(e.amount||0),0,100);if(e.hidden){const record=save.career.hiddenConsequences?.find(item=>item.id===e.id);if(record){record.revealed=true;record.appliedSeason=save.career.season}}}return due;
}
...
export function eventDiagnostics(save){
  ...
  return{available:mem.lastAvailableCount,filtered:mem.lastFilteredCount,generated:mem.generatedCount,recentCount:recent.length,recentUnique:unique,recentRepeatRate:recent.length?Number(((recent.length-unique)/recent.length*100).toFixed(1)):0,typeCounts:{...mem.typeCounts},chainsOpen:[...mem.chainsOpen],chainsStarted:[...mem.chainsStarted],chainsClosed:[...mem.chainsClosed],chainCompletionRate:mem.chainsStarted.length?Number((mem.chainsClosed.length/mem.chainsStarted.length*100).toFixed(1)):0,cooldowns:Object.keys(mem.cooldowns).length,recentPersons:[...mem.recentPersons.slice(-6)]};
}
