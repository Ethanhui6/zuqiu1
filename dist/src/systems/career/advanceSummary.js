...
export function createAdvanceSummaryViewModel(result){
  ...
  const entries=[['球队比赛',delta.processedMatchCount],['出场',delta.matchesPlayed],['进球',delta.goals],['助攻',delta.assists],['平均评分',delta.averageRating||null],['能力',delta.ovrChange],['教练信任',delta.coachTrustDelta],['粉丝',delta.fansDelta],['身价',delta.marketValueDelta]];
  for(const [label,value] of entries){
    ...
    const display=label==='身价'?signedMoney(value):['能力','教练信任','粉丝'].includes(label)?signed(value):value;
