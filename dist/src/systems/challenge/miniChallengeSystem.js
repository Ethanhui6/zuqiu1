...
export function resolveMiniChallenge(save,match){if(!match?.miniChallenge||!match.playerResult)return null;const group=POSITION_CONFIG[save.player.position]?.group||'attack',definition=(POOLS[group]||POOLS.attack).find(x=>x.id===match.miniChallenge.id),completed=Boolean(definition?.check(match.playerResult));match.miniChallenge.completed=completed;if(completed){save.status.coachTrust=Math.min(100,save.status.coachTrust+2);save.player.xp[save.player.position==='GK'?'pac':'pas']=(save.player.xp[save.player.position==='GK'?'pac':'pas']||0)+8}return match.miniChallenge}
