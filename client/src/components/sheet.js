...
export function closeSheet(){
  ...
  const remove=()=>{
    if(removed)return;
    removed=true;
    oldBackdrop.remove();closing=false;
...
export function openSheet({title,subtitle='',content,actions=[],dismissible=true,size='normal'}){
  if(backdrop){
    const old=backdrop;backdrop=null;current=null;cleanupListeners();old.setAttribute('aria-hidden','true');old.remove();unlockPageScroll();
  }
  ...
  const host=document.querySelector('#overlay-root')||document.body;
  backdrop.append(current);host.append(backdrop);
  applyViewportHeight();
