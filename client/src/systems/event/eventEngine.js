...
function ensureMemory(save){
  ...
  mem.triggered??=[];mem.recentTags??=[];mem.recentCategories??=[];mem.recentTitles??=[];mem.recentTemplateTitles??=[];mem.recentChoiceSignatures??=[];mem.choices??=[];mem.chainsOpen??=[];mem.chainsStarted??=[];mem.chainsClosed??=[];mem.cooldowns??={};mem.recentEventIds??=[];mem.recentPersons??=[];mem.recentOpponents??=[];mem.typeCounts??={};mem.generatedCount??=0;mem.duplicateCount??=0;mem.lastAvailableCount??=0;mem.lastFilteredCount??=0;
  return mem;
}
...
export function resolveEventChoice(save,choiceId){
  ...
  const mem=ensureMemory(save);if(!mem.triggered.includes(event.id))mem.triggered.push(event.id);mem.recentTags.push(event.category,...event.tags.slice(0,2));mem.recentTags=mem.recentTags.slice(-20);mem.recentCategories.push(event.category);mem.recentCategories=mem.recentCategories.slice(-20);mem.recentTitles.push(titleKey(event.title));mem.recentTitles=mem.recentTitles.slice(-20);mem.recentTemplateTitles.push(titleKey(event.templateTitle||event.title));mem.recentTemplateTitles=mem.recentTemplateTitles.slice(-20);mem.recentEventIds.push(event.id);mem.recentEventIds=mem.recentEventIds.slice(-20);mem.recentPersons.push(event.person||PERSON_BY_CATEGORY[event.category]||'职业环境');mem.recentPersons=mem.recentPersons.slice(-20);const signature=choiceSignature(event.choices);mem.recentChoiceSignatures.push(signature);mem.recentChoiceSignatures=mem.recentChoiceSignatures.slice(-20);mem.choices.push({eventId:event.id,title:event.title,category:event.category,choiceId,label:choice.text,outcome:outcome.label,season:save.career.season,month:save.career.month,week:save.career.calendar?.week||1});mem.choices=mem.choices.slice(-800);mem.cooldowns[event.id]=save.career.season+event.cooldown;mem.lastChoiceSignature=signature;
