...
function createFixture({save,current,opponent,competition,index,rng,week}){
  ...
  return{
    ...
    season:save.career.season,week,round:index+1,roundLabel:competition.type==='cup'?`第${index+1}轮`:competition.type==='continental'?`第${index+1}比赛日`:`第${index+1}轮`,competition:competition.name,competitionType:competition.type,
...
export function generateSeasonSchedule(save,repo,{force=false}={}){
  ...
  const assigned=assignWeeks(rng.shuffle(specs).slice(0,36),rng).map(spec=>createFixture({save,current,...spec,index:spec.index,rng,week:spec.week}));
...
export function settleCompetitionState(save,repo){
  ...
  const rng=keyedRandom(save.rng.seed,'league-state',save.career.season,current.id),stats=save.career.seasonStats;
