...
export function renderOnboarding(root,{repo,onComplete,onCancel=null}){
  ...
  function positionStep(){
    ...
    Object.entries(POSITION_CONFIG).forEach(([id,config])=>{
      const positionButton=button(config.name,{className:`pitch-position ${draft.position===id?'is-selected':''}`,onClick:()=>{draft.position=id;draft.style=config.roles[0];draft.talents=[];draft.academyOffers=[];persist();render();}});
  ...
  function talentStep(){
    ...
    const wrap=el('div',{className:'talent-layout'}),toolbar=el('div',{className:'talent-toolbar'},[
      ...
      button('重新抽取',{className:'button button--secondary',disabled:draft.rerolls<=0,onClick:()=>{if(draft.rerolls<=0)return;draft.rerolls-=1;prepareTalents(true);persist();render();}})
    ...
    draft.talents.forEach((talent,index)=>{
      ...
      grid.append(createTalentCard(talent,{selected:draft.selectedTalent===index,position:draft.position,style:draft.style,attrs,onSelect:()=>{draft.selectedTalent=index;draft.academyOffers=[];persist();render();}}));
    });
