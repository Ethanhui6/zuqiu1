...
export function morePage(app,state){
  ...
  root.addEventListener('click',e=>{const key=e.target.closest('[data-more]')?.dataset.more;if(!key)return;({simulation:()=>app.openSimulation(),world:()=>app.navigate('transfer'),leaderboard:()=>app.openLeaderboard(),career:()=>app.openCareerHub(),share:()=>app.openCareerShare(),honors:()=>app.openHonors(),analytics:()=>app.openAnalytics(),medical:()=>app.openMedical(),locker:()=>app.openLocker(),contract:()=>app.openContract(),settings:()=>app.openSettings()})[key]?.();});
  return root;
}
