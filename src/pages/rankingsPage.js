...
export function renderRankingsPage(container,ctx){
  ...
  clear(container);
  const page=el('section',{className:'page v20-rankings-page'}),title=el('header',{className:'v20-page-intro'},[el('div',{},[el('span',{className:'eyebrow',text:'生涯排行榜'}),el('h1',{text:'本地成绩与服务器认证成绩分开记录'}),el('p',{text:'离线存档始终可进入本地榜；正式世界榜只显示服务器按连续动作日志重新结算的成绩。'})])]);
  const current=currentScoreCard(evidence,score,save.meta?.ranking),localSection=el('section',{className:'v20-ranking-section'}),localHead=sectionHeading('本地存档榜','设备内全部存档 · 支持筛选和排序'),localControls=el('div',{className:'v20-ranking-filter-bar'}),localList=el('div',{className:'v20-ranking-list'});
  ...
  const worldSection=el('section',{className:'v20-ranking-section'}),worldHead=sectionHeading('联网世界榜','服务端权威计分 · 可分页、筛选并查看公开生涯摘要'),worldActions=el('div',{className:'v20-ranking-actions'}),refreshButton=button('刷新',{className:'button button--secondary'}),submitButton=button('发布当前生涯',{className:'button button--primary'}),privacyButton=button('公开信息',{className:'button',disabled:save.meta?.ranking?.status!=='verified'}),worldStatus=el('p',{className:'v20-ranking-status',attrs:{'aria-live':'polite'},text:'正在连接世界榜…'}),categoryBar=el('div',{className:'achievement-filters v20-ranking-categories'}),worldList=el('div',{className:'v20-ranking-list'}),pager=el('nav',{className:'pager'});
  ...
  WORLD_CATEGORIES.forEach(([id,label])=>categoryBar.append(button(label,{className:`v20-filter-chip ${id===category?'is-selected':''}`,onClick:event=>{category=id;worldPage=1;categoryBar.querySelectorAll('button').forEach(item=>item.classList.toggle('is-selected',item===event.currentTarget));void loadWorld()}})));
...
function currentScoreCard(evidence,score,ranking){
  ...
  return el('section',{className:'v20-current-score-card'},[el('div',{className:'v20-current-score-main'},[el('span',{className:'v20-score-grade',text:score.grade}),el('strong',{text:formatNumber(score.total)}),el('small',{text:'满分10000'})]),el('div',{className:'v20-score-player'},[el('h2',{text:evidence.player.name}),el('p',{text:`${POSITION_CONFIG[evidence.player.position]?.name||'球员'} · ${evidence.career.clubName} · 第${evidence.career.season}赛季`}),el('span',{className:`v20-verification-pill ${ranking?.status==='verified'?'is-verified':''}`,text:ranking?.status==='verified'?'服务器已认证':ranking?.eligible?'服务器日志跟踪中':'仅本地计分'})]),el('div',{className:'v20-score-breakdown'},items.map(([label,value])=>el('div',{className:'v20-score-part'},[el('small',{text:label}),el('strong',{text:formatNumber(value)})]))) ]);
}
function rankingRow(entry,rank,{local=false}={}){
  ...
  const footer=local
    ?el('span',{className:`v20-verified-mark ${certified?'':'is-local'}`,text:certified?'已认证':'仅本地'})
    :el('div',{className:'v20-ranking-entry-actions'},[
      el('span',{className:'v20-verified-mark',text:'已认证'}),
  ...
  return el('article',{className:`v20-ranking-row ${entry.current?'is-current':''}`},[el('span',{className:`v20-ranking-rank rank-${rank}`,text:rank<=3?['🥇','🥈','🥉'][rank-1]:String(rank)}),el('div',{className:'v20-ranking-player'},[el('strong',{text:name}),el('small',{text:`${club} · ${details}`}),local?el('small',{text:`创建 ${dateTime(entry.createdAt)} · 最后游玩 ${dateTime(entry.updatedAt)}`}):null]),el('div',{className:'v20-ranking-score'},[el('strong',{text:formatNumber(score)}),el('small',{text:`${grade}级`})]),footer]);
}
...
function sectionHeading(title,copy){return el('header',{className:'v20-ranking-heading'},[el('div',{},[el('h2',{text:title}),el('p',{text:copy})])])}
...
function empty(value){return el('div',{className:'v20-ranking-empty'},[el('span',{text:'◎'}),el('p',{text:value})])}
function openPrivacySettings(save,trigger,reload){
  const ranking=save.meta?.ranking,input=el('input',{className:'field-control',attrs:{type:'text',maxlength:'24',value:save.player.displayName||save.player.name,'aria-label':'公开昵称'}}),visible=el('input',{attrs:{type:'checkbox',checked:true}}),content=el('div',{className:'v20-settings-card'},[el('label',{className:'v20-setting-row'},[el('div',{},[el('strong',{text:'公开昵称'}),el('small',{text:'不会公开邮箱、IP、设备标识、真实姓名或精确位置。'})]),input]),el('label',{className:'v20-toggle-row'},[el('span',{text:'公开生涯详情'}),visible])]);
