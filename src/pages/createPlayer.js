...
export function createPlayerWizard(app){
  ...
  const body=()=>{
    if(step===0)return `<div class="form-grid">${field('姓名','name',draft.name,'text','例如：林越')}${field('球衣名','shirtName',draft.shirtName,'text','例如：LIN')}${field('国家或地区','country',draft.country)}${field('身高（cm）','height',draft.height,'number')}${field('体重（kg）','weight',draft.weight,'number')}${selectField('惯用脚','foot',['右脚','左脚','双足'],draft.foot)}${field('球衣号码','number',draft.number,'number')}<div class="field field-note"><span class="field-label">年龄与生日</span><strong>系统自动生成 16 岁档案</strong><small>生日会由本次存档种子稳定生成</small></div></div>`;
  ...
  const bind=()=>{
    ...
    root.querySelector('[data-next]')?.addEventListener('click',()=>{
      ...
      const club=academyClubs(draft.position).find(c=>c.id===draft.club)||academyClubs(draft.position)[0];
      const state=app.store.get(),birth=generatedBirth(`${state.createdAt}:${draft.name}:${draft.country}`);
      const player={...draft,birthDate:birth,club:club.name,clubId:club.id,team:'青年队',age:16,stats:report.stats,potential:report.potential,ovr:computeOverall(report.stats,draft.position),fatigue:18,morale:72,fitness:84,coachTrust:52,status:'健康',previousStats:{...report.stats},squad:dataRepository.registry?.rosterForClub(club.id,{seed:draft.name||'career-start'})||[]};
      app.store.set(s=>{s.player=player;s.career.history.push({date:s.simulation.date,type:'签约',text:`加入${club.name}青年队`});s.news.items.unshift({id:`welcome-${s.createdAt}`,date:s.simulation.date,type:'生涯',title:'青训生涯正式开始',copy:`${draft.name} 以 16 岁身份加入${club.name}青年队。`,read:false});return s;});
...
function generatedBirth(seed){const rnd=seeded(seed);const month=1+Math.floor(rnd()*12);const day=1+Math.floor(rnd()*28);return `2009-${String(month).padStart(2,'0')}-${String(day).padStart(2,'0')}`;}
