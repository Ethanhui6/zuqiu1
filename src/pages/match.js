...
import { getMatchInteractionsForPosition } from '../core/matchInteractions.js';
...
export function matchPage(app,state){
  ...
  const interactions=getMatchInteractionsForPosition(state.player?.position);
  ...
  let tactic='balanced';
  let interaction=interactions[0]?.id||'shooting';
  ...
  root.querySelectorAll('[data-match-interaction]').forEach(el=>el.onclick=()=>{interaction=el.dataset.matchInteraction;root.querySelectorAll('[data-match-interaction]').forEach(x=>x.classList.toggle('active',x===el));app.feedback.emit('matchInteraction',interactions.find(item=>item.id===interaction).name);});
