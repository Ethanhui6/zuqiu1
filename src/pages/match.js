...
export function matchPage(app,state){
  ...
  ${match?`${matchPreview(match,currentSnapshot,opponentSnapshot,state,playerStatus)}<div class="page-fixed-action match-fixed-action"><span><small>${playerStatus.available === false ? '当前不可出场' : match.date === state.simulation.date ? '比赛已经开始' : match.date}</small><strong>${playerStatus.available === false ? playerStatus.label : match.competition || '下一场比赛'}</strong></span><button class="app-button primary match-enter-button" data-play ${playerStatus.available === false ? 'disabled' : ''}>${icon('play','sm')}${playerStatus.available === false ? '无法进入' : '进入比赛'}</button></div>`:emptyState('暂无即将进行的比赛','返回生涯首页查看下一关键节点。','calendar')}
  ...
  const lineup=root.querySelector('.match-preview__lineups');
  if(lineup){const details=document.createElement('details');details.className='match-preview__lineups';const summary=document.createElement('summary');summary.innerHTML='<strong>预计阵容与阵型</strong><span>点击查看双方 22 人名单</span>';const list=document.createElement('div');list.className='match-preview__lineup-list';while(lineup.firstChild)list.append(lineup.firstChild);details.append(summary,list);lineup.replaceWith(details);}
