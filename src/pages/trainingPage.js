...
export function renderTrainingPage(container,ctx){
  ...
  const grid=el('div',{className:'training-grid'});getTrainingPlans(save).forEach(plan=>{
    ...
    card.addEventListener('click',()=>{store.update(s=>selectTrainingPlan(s,plan.id),'training-selected');grid.querySelectorAll('.training-card').forEach(x=>x.classList.toggle('is-selected',x.dataset.planId===plan.id));currentLabel.textContent=plan.name;showToast(`下一次训练：${plan.name}`,{type:'success'})});grid.append(card);
