...
function autoPausePanel(save,store){
  const panel=el('details',{className:'settings-details'},[el('summary',{text:'自动暂停规则'})]),grid=el('div',{className:'toggle-grid'});
  Object.entries(AUTO_PAUSE_RULES).forEach(([key,label])=>{
    const input=el('input',{attrs:{type:'checkbox',checked:save.settings.pace?.autoPause?.[key]!==false}});
