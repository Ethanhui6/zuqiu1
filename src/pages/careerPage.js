...
export function renderCareerPage(container,ctx){
  ...
  page.append(
    el('div',{className:'career-hero'},[createPlayerCard(save,club),careerOverview(save,club,repo)]),
    objectivePanel(save,store,ctx),
    advancePanel(save,repo,runAdvance),
    lastSummary(save),
    quickCards(save),
    compactFacilities(save,club,ctx)
  );
  ...
  function chooseEvent(choice,resumeTarget){
    try{
