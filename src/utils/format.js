...
const LABELS={injuryRisk:'受伤风险',coachTrust:'教练信任',relationship:'人际关系',morale:'士气',fitness:'体能',reputation:'声望',matchRating:'比赛评分',transferInterest:'转会关注',totalScore:'游戏评分',grade:'生涯等级',verificationStatus:'验证状态'};
export function formatStatLabel(key){return LABELS[key]||String(key||'数据')}
export function formatStatValue(value){return safeText(value)}
export function formatEffectList(value){return Array.isArray(value)?value.map(item=>safeText(item)).join('、'):safeText(value)}
export function formatClubName(club){return safeText(club?.cn||club?.name||club,'未知俱乐部')}
export function formatPercentage(value){return `${Math.round(Number(value)||0)}%`}
