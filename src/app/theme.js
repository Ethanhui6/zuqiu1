...
let systemQuery;
export function getTheme(){const saved=localStorage.getItem(KEY);return THEME_MODES.includes(saved)?saved:'dark'}
...
export function applyTheme(preference=getTheme()){const mode=THEME_MODES.includes(preference)?preference:'dark';const resolved=resolveTheme(mode);document.documentElement.dataset.theme=resolved;document.documentElement.dataset.themePreference=mode;document.documentElement.style.colorScheme=resolved;localStorage.setItem(KEY,mode);watchSystemTheme();return resolved}
