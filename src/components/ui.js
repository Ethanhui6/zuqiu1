...
export class OverlayManager {
  constructor(root){ this.root=root; this.active=null; this.startY=0; this.escapeHandler=null; this.lastFocus=null; }
  close(){ if(!this.active)return; const el=this.active; const focus=this.lastFocus; if(this.escapeHandler)document.removeEventListener('keydown',this.escapeHandler); this.escapeHandler=null; el.style.opacity='0'; setTimeout(()=>{el.remove(); if(this.active===el){this.active=null;document.body.classList.remove('has-open-sheet');focus?.focus?.();}},150); }
  sheet(title,content,{wide=false,onMount=null}={}){
    this.close(); this.lastFocus=document.activeElement; document.body.classList.add('has-open-sheet'); const overlay=document.createElement('div'); overlay.className='overlay';
    overlay.innerHTML=`<section class="sheet ${wide?'wide':''}" role="dialog" aria-modal="true" aria-label="${title}"><div class="sheet-handle"></div><header class="sheet-head"><h2 class="sheet-title">${title}</h2><button class="icon-button" data-close-sheet aria-label="关闭">${icon('close')}</button></header><div class="sheet-body">${content}</div></section>`;
    overlay.addEventListener('click',e=>{ if(e.target===overlay||e.target.closest('[data-close-sheet]')) this.close(); });
    const sheet=overlay.querySelector('.sheet');
    sheet.addEventListener('touchstart',e=>{this.startY=e.touches[0].clientY;},{passive:true});
    sheet.addEventListener('touchend',e=>{if(e.changedTouches[0].clientY-this.startY>90 && sheet.scrollTop<=0)this.close();},{passive:true});
    this.escapeHandler=e=>{if(e.key==='Escape')this.close();}; document.addEventListener('keydown',this.escapeHandler);
    this.root.append(overlay); this.active=overlay; overlay.querySelector('[data-close-sheet]')?.focus(); onMount?.(overlay); return overlay;
  }
