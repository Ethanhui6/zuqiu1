...
export class OverlayManager {
  constructor(root){ this.root=root; this.active=null; this.startY=0; }
  close(){ if(!this.active)return; const el=this.active; el.style.opacity='0'; setTimeout(()=>{el.remove(); if(this.active===el)this.active=null;},150); }
  sheet(title,content,{wide=false,onMount=null}={}){
    this.close(); const overlay=document.createElement('div'); overlay.className='overlay';
    ...
    document.addEventListener('keydown',this.escapeHandler={handleEvent:(e)=>{if(e.key==='Escape')this.close();}},{once:true});
    this.root.append(overlay); this.active=overlay; onMount?.(overlay); return overlay;
  }
