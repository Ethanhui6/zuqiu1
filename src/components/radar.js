const KEYS=[['speed','??'],['shooting','??'],['passing','??'],['dribbling','??'],['defending','??'],['physical','??']];
...
export function radarChart(current={},previous={},potential=80){
  ...
  return `<div class="radar-wrap"><svg class="radar" viewBox="0 0 220 220" role="img" aria-label="???????">${rings}${axes}<polygon class="radar-potential" points="${polygon(KEYS.map(()=>potential))}"/><polygon class="radar-prev" points="${polygon(KEYS.map(([key])=>previous[key]??current[key]??0))}"/><polygon class="radar-current" points="${polygon(KEYS.map(([key])=>current[key]??0))}"/>${labels}</svg></div>`;
}
