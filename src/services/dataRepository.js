...
export class DataRepository{
  async init(){
    const nameFiles=['china','japan','south-korea','england','scotland','wales','ireland','france','germany','spain','portugal','italy','netherlands','belgium','brazil','argentina','usa','mexico','vietnam','thailand','saudi-arabia','turkey','nigeria','ghana','senegal','morocco','egypt','other'];
    const [clubs,expansion,legacyClubs,templates,achievements,positions,eventIndex,storyChains,version,players,trophies,legacyTrophies,sources,careerEvents,positionEvents,...names]=await Promise.all([
      json('./data/clubs.json'),json('./data/world-expansion.json'),json('./data/legendevo-clubs.json'),json('./data/legend-templates.json'),json('./data/achievements.json'),json('./data/positions.json'),json('./data/events/index.json'),json('./data/events/story-chains.json'),json('./data/version.json'),json('./data/players.json'),json('./data/trophies.json'),json('./data/legendevo-trophies.json'),json('./data/data-sources.json'),json('./data/events/career-events.json'),json('./data/events/position-events.json')
      ,...nameFiles.map(file=>json(`./data/names/${file}.json`))
    ]);
    const currentClubs=[...(clubs.clubs||clubs),...(expansion.clubs||[])];
    const legacyNames=new Map((legacyClubs.sourceClubs||[]).map(item=>[item.currentClubId,[item.id,item.name,item.short_name,item.name_zh].filter(Boolean)]));
    this.clubs=this.enrichClubs([...currentClubs.map(club=>({...club,aliases:[...new Set([...(club.aliases||[]),...(legacyNames.get(club.id)||[])])]})),...(legacyClubs.clubs||[])]);this.leagues=[...(clubs.leagues||[]),...(expansion.leagues||[]),...(legacyClubs.leagues||[])];this.templates=templates;this.achievements=achievements;this.positions=positions;this.eventIndex=eventIndex;this.storyChains=Array.isArray(storyChains)?storyChains:(storyChains.events||[]);this.version=version;this.sources=sources;
    ...
    this.registry=createWorldRegistry({clubs:this.clubs,leagues:this.leagues,players,trophies:[...trophies,...(expansion.trophies||[]),...legacyTrophies],competitions:expansion.competitions||[],nameProfiles:this.nameProfiles});this.audit=this.registry.audit;
