...
export class SaveManager{
  ...
  peek(id){
    if(!id)return null;
    const primary=safeParse(localStorage.getItem(slotKey(id)));
    if(primary&&integrity(primary).ok){const prepared=prepare(primary);if(validPlayable(prepared))return prepared}
    const backup=safeParse(localStorage.getItem(backupKey(id)));
    if(backup&&integrity(backup).ok){const prepared=prepare(backup);if(validPlayable(prepared))return prepared}
    return null;
  }
