...
export function buildDefaultSave(){
  return{
    schemaVersion:SAVE_SCHEMA,gameVersion:APP_VERSION,createdAt:Date.now(),updatedAt:Date.now(),rng:{seed:createSeed(),state:0,counter:0},settings:{theme:'light',reducedMotion:false,animationMode:'standard',sound:true,haptics:true,pace:{mode:'standard',speed:'normal',eventAnimationSpeed:'standard',autoTraining:true,autoMatch:true,autoPause:{...DEFAULT_AUTO_PAUSE}}},
...
export function normalizeSave(save){
  ...
  save.settings={theme:'light',reducedMotion:false,animationMode:'standard',sound:true,haptics:true,...(save.settings||{})};save.settings.pace={mode:'standard',speed:'normal',eventAnimationSpeed:'standard',autoTraining:true,autoMatch:true,autoPause:{...DEFAULT_AUTO_PAUSE},...(save.settings.pace||{})};save.settings.pace.autoPause={...DEFAULT_AUTO_PAUSE,...(save.settings.pace.autoPause||{})};
  ...
  save.career.facilities={visits:[],locks:{},...(save.career.facilities||{})};save.career.facilities.visits??=[];save.career.facilities.locks??={};save.career.facilityCenter={medicalPlan:'light',medicalHistory:[],lockerHistory:[],analysisViewedAt:null,honoursViewedAt:null,lastLockerWeek:null,...(save.career.facilityCenter||{})};save.career.facilityCenter.medicalHistory??=[];save.career.facilityCenter.lockerHistory??=[];
  const legacyWeek=Math.max(1,Math.min(52,Number(save.career.calendar?.week||((Number(save.career.month||1)-1)*4+1))));const bounds=seasonBounds(Number(save.career.year||2026));const currentDate=save.career.gameClock?.currentDate||addGameDays(bounds.seasonStartDate,(legacyWeek-1)*7);save.career.gameClock={currentDate,seasonId:save.career.gameClock?.seasonId||bounds.seasonId,seasonStartDate:save.career.gameClock?.seasonStartDate||bounds.seasonStartDate,seasonEndDate:save.career.gameClock?.seasonEndDate||bounds.seasonEndDate,competitionWeek:Number(save.career.gameClock?.competitionWeek||legacyWeek),calendarWeek:Number(save.career.gameClock?.calendarWeek||1),currentPhase:save.career.gameClock?.currentPhase||'regular-season',currentTransferWindow:save.career.gameClock?.currentTransferWindow??null,lastProcessedDate:save.career.gameClock?.lastProcessedDate||currentDate};save.career.calendar={week:legacyWeek,absoluteWeek:Math.max(1,(Number(save.career.season||1)-1)*52+legacyWeek),nextEventWeek:1,nextEventDate:null,...(save.career.calendar||{})};save.career.weekState={trainingDone:false,eventDone:false,matchDone:false,trainingResult:null,...(save.career.weekState||{})};save.career.schedule??=null;save.career.strategies={...DEFAULT_STRATEGIES,...(save.career.strategies||{})};save.career.objectives={season:save.career.season,cycleId:0,generatedDate:null,expiresDate:null,candidates:[],active:[],completed:[],rewarded:[],...(save.career.objectives||{})};for(const key of ['candidates','active','completed','rewarded'])save.career.objectives[key]??=[];save.career.advance={running:false,lastSummary:null,history:[],resumeTarget:null,activeRequest:null,...(save.career.advance||{})};save.career.advance.history??=[];save.career.weeklyPlan={selected:'balanced',appliedWeeks:[],history:[],...(save.career.weeklyPlan||{})};save.career.aiDirector={lastReviewDate:null,nextReviewDate:null,focus:null,recommendations:[],eventBias:{},history:[],...(save.career.aiDirector||{})};save.career.squadCompetition={seasonId:null,competitors:[],rank:4,lastUpdatedDate:null,...(save.career.squadCompetition||{})};save.career.agent=save.career.agent||null;save.career.messages={items:[],seen:[],lastGeneratedDate:null,...(save.career.messages||{})};save.career.formMomentum={value:0,history:[],...(save.career.formMomentum||{})};save.career.traits={progress:{},unlocked:[],...(save.career.traits||{})};save.career.worldDynamics={lastMonth:null,items:[],clubStates:{},...(save.career.worldDynamics||{})};save.career.worldExplorer={level:'world',continent:null,country:null,leagueId:null,query:'',filters:{},scroll:{},favorites:[],compare:[],transferTargets:[],...(save.career.worldExplorer||{})};for(const key of ['favorites','compare','transferTargets'])save.career.worldExplorer[key]??=[];save.career.worldExplorer.filters??={};save.career.worldExplorer.scroll??={};save.career.trainingEvents={current:null,history:[],resolvedWeeks:[],...(save.career.trainingEvents||{})};save.career.trainingEvents.history??=[];save.career.trainingEvents.resolvedWeeks??=[];save.career.ui={attention:{read:{},lastViewed:{}},details:{},scrollPositions:{},...(save.career.ui||{})};save.career.ui.attention={read:{},lastViewed:{},...(save.career.ui.attention||{})};save.career.ui.details??={};save.career.ui.scrollPositions??={};save.career.matchHistory??=[];save.career.lastMatchResult??=null;save.career.majorNodes??=[];
  ...
  save.career.eventMemory={triggered:[],recentEventIds:[],recentTags:[],recentCategories:[],recentTitles:[],recentTemplateTitles:[],recentChoiceSignatures:[],recentFingerprints:[],recentPersons:[],recentOpponents:[],choices:[],chainsOpen:[],chainsStarted:[],chainsClosed:[],cooldowns:{},typeCounts:{},generatedCount:0,duplicateCount:0,lastAvailableCount:0,lastFilteredCount:0,lastChoiceSignature:'',...(save.career.eventMemory||{})};
  for(const key of ['triggered','recentEventIds','recentTags','recentCategories','recentTitles','recentTemplateTitles','recentChoiceSignatures','recentFingerprints','recentPersons','recentOpponents','choices','chainsOpen','chainsStarted','chainsClosed'])save.career.eventMemory[key]??=[];
...
export function migrateV19ToV20(raw){return migrateLegacy(raw)}
export function migrateV22ToV23(raw){return migrateLegacy(raw)}
export function migrateV20ToV22(raw){return migrateLegacy(raw)}
export function migrateV21ToV22(raw){return migrateLegacy(raw)}
