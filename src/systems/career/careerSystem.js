...
export function createTalentCandidates({seed,position,style,templates,count=3}){
  ...
  for(let i=0;i<count;i++){
    const roll=rng.next()*100;const rarity=roll<1?'legend':roll<8?'elite':roll<30?'good':'common';const cfg=TALENT_RARITY[rarity];
...
export function generateAcademyOffers({seed,nation,position,ovr,talent,clubs}){
  ...
  return picked.map(({club,score},i)=>({clubId:club.id,squad:i===0&&talent.rarityKey==='legend'?'U19青年队':'U18青年队',role:i===0?'重点培养对象':i===1?'地区希望之星':'试训球员',contractYears:2,weeklyWage:Math.round(120+club.finance*4+score*2),reason:club.needs.includes(position)?'该位置是青训重点补强方向':'青训体系与成长风格匹配'}));
}
...
export function createNewSave(draft,club,slotId){
  ...
  const save={schemaVersion:18,gameVersion:'18.0.0',createdAt:Date.now(),updatedAt:Date.now(),rng:{...rng.snapshot()},settings:{theme:'system',reducedMotion:false},
