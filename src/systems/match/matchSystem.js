...
import {ensureSchedule,fixturesForWeek,nextFixture,markFixturePlayed} from '../schedule/scheduleSystem.js';
...
function fixtureForCurrentWeek(save,repo){const week=save.career.calendar?.week||1;return fixturesForWeek(save,repo,week)[0]||nextFixture(save,repo,{fromWeek:week})}
...
export function generateMatch(save,repo,{fixtureId}={}){
  ...
  const weather=rng.pick(WEATHER),youth=save.career.squadLevel!=='一线队',playerActive=save.career.teamRole!=='未进入名单'&&!save.status.injury&&save.status.suspension<=0;
  const baseChance=clamp((youth?.7:.4)+(save.player.ovr-current.rep)*.018+(save.status.coachTrust-50)*.004,0.12,.94),starts=playerActive&&rng.bool(baseChance),substitute=playerActive&&!starts&&rng.bool(.68),minute=starts?1:substitute?rng.int(52,78):null;
  ...
  const match={id:`M-${fixture.id}-${rng.state}`,fixtureId:fixture.id,week:fixture.week,competition:fixture.competition,competitionType:fixture.competitionType,opponentId:opp.id,home:fixture.home,weather,importance:fixture.importance,starts,substitute,minute,keyChoices,choice:null,resolved:false,timeline:[],score:null,playerResult:null,presentation:null,generatedRngState:rng.state};
  save.career.pending.match=match;save.rng=rng.snapshot();return match;
}
...
export function resolveMatch(save,repo,choiceId,{presentation='interactive'}={}){
  ...
  if(choice&&playerActive){const skill=save.player.attrs[choice.focus]||60,chance=clamp(.34+(skill-55)*.009+fitness*.02+morale*.02-(save.status.fatigue*.002),.08,.88);if(rng.bool(chance)){contribution=1;teamXg+=.38}else if(rng.bool(.25)){contribution=-1;oppXg+=.22}}
